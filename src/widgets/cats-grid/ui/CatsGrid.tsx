...
export const CatsGrid = ({ cats, isLoading = false, skeletonCount = 8 }: CatsGridProps) => {
  if (isLoading) {
    return (
      <div className={styles.grid} role="status" aria-live="polite" aria-label="Loading cat cards">
  ...
  if (cats.length === 0) {
    return <p className={styles.empty}>... загружаем еще котиков ...</p>;
  }

  return (
    <div className={styles.grid} role="list" aria-label="Cats grid">
      {cats.map((cat) => (
        <CatCard key={cat.id} cat={cat} action={<div />} />
      ))}
    </div>
  );
};
